...
def answer(call: VoIPCall):
    try:
        ...
        chunk_8khz, state = audioop.ratecv(y.tobytes(), 2, 2, sr, 8000, None)
        chunk_ulaw = audioop.lin2ulaw(chunk_8khz, 4)

        call.answer()

        call.write_audio(chunk_ulaw)

        stop = time.time() + (len(chunk_ulaw) / 8000)
